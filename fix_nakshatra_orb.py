...
new_painter = """class _UnifiedTrifectaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxR = (math.min(w, h) / 2) - 45.0; // Space for labels
    ...
    canvas.drawLine(c, getPt(aPitta, maxR * 1.2), axisPaint);
    canvas.drawLine(c, getPt(aKapha, maxR * 1.2), axisPaint);
    canvas.drawLine(c, getPt(aVata, maxR * 1.2), axisPaint);
    ...
    Path _createAuraPath(double rP, double rK, double rV, double shift) {
      final path = Path();
      const points = 180;
      for (int i = 0; i <= points; i++) {
        ...
        // Blend 80% circle, 20% triangle distance
    ...
    final basePathOrb = _createAuraPath(rBasePitta, rBaseKapha, rBaseVata, 2.0);
    ...
    // Drama based on total shift, minimum 6.0
    double totalShift = ((curPitta-basePitta).abs() + (curKapha-baseKapha).abs() + (curVata-baseVata).abs()).toDouble();
    double drama = 6.0 + (totalShift * 0.1).clamp(0.0, 10.0);
    
    final curPathOrb = _createAuraPath(rCurPitta, rCurKapha, rCurVata, drama);
    ...
    void drawAuraCircle(double angle, double r, Color col) {
      ...
      // Large and minimal as requested
      double circleR = 24.0; 
    ...
    void drawLabel(String elem, String dosha, int curVal, int baseVal, double angle, Color col) {
      ...
      double circleR = 24.0;
      double lr = rCur + circleR + 20; // Push outside the big circles
