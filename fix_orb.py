...
new_painter = """class _UnifiedTrifectaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxR = 65.0; // Base size for the visualization
    ...
    canvas.drawLine(c, Offset(c.dx + maxR * 1.5 * math.cos(aPitta), c.dy + maxR * 1.5 * math.sin(aPitta)), axisPaint);
    canvas.drawLine(c, Offset(c.dx + maxR * 1.5 * math.cos(aKapha), c.dy + maxR * 1.5 * math.sin(aKapha)), axisPaint);
    canvas.drawLine(c, Offset(c.dx + maxR * 1.5 * math.cos(aVata), c.dy + maxR * 1.5 * math.sin(aVata)), axisPaint);

    // Calculate radii based on percentages
    double getR(int val) => maxR * (val / 100.0).clamp(0.1, 1.5) * 1.5;
    ...
    // 2. The "Triangle In Between" - Baseline (Prakriti)
    final basePath = Path()
    ...
    // Fill the baseline triangle faintly
    canvas.drawPath(basePath, Paint()
      ..color = fgMuted.withAlpha(10)
      ..style = PaintingStyle.fill
      ..isAntiAlias = true
    );

    // 3. The "Wavy Circular Orb" - Current State (Vikriti)
    // Smooth polar interpolation between the 3 current radii
    double getSmoothRadius(double t) {
      ...
      double f = (t - a0) / (a1 - a0);
      double smoothF = f * f * (3 - 2 * f); // Smoothstep for organic wavy curve
      return r0 + smoothF * (r1 - r0);
    }
    ...
    // 4. Draw Nodes at the 3 Current Peaks
    void drawDot(Offset pt, Color col) {
      canvas.drawCircle(pt, 4, Paint()..color = col..style=PaintingStyle.fill..isAntiAlias=true);
      canvas.drawCircle(pt, 10, Paint()..color = col.withAlpha(40)..style=PaintingStyle.stroke..strokeWidth=2..isAntiAlias=true);
    }
    ...
    // 5. Extremely Clear Labels
    void drawLabel(String elem, String dosha, int curVal, int baseVal, double angle, Color col) {
      // Dynamic label distance based on the orb's stretch
      double rCur = getR(curVal);
      double rBase = getR(baseVal);
      double maxBranchR = math.max(rCur, rBase);
      double lr = maxBranchR + 30; 
