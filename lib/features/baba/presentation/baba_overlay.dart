...
class _BabaOverlayState extends State<BabaOverlay>
  ...
  static const double _activeSize = 140.0;
  ...
  void _showRecent() {
    ...
    appRouter.push(RouteNames.recentConversations, extra: {
      'onConversationSelected': (String conversationId) {
        appRouter.push(RouteNames.aiChat, extra: {'conversationId': conversationId});
      },
    });
  }
  ...
  /// The centred, full call UI (unchanged behaviour).
  Widget _buildCallPresence(BuildContext context, bool isDark) {
    ...
    return AnimatedAlign(
      ...
      child: AnimatedPadding(
        padding: EdgeInsets.only(bottom: 16 + _safeBottom + _barInset),
        duration: const Duration(milliseconds: 500),
        curve: Curves.easeOutCubic,
        child: Column(
          ...
          children: [
            AnimatedSize(
              ...
              child: reply.isNotEmpty
                  ? _buildCaption(reply, isDark, _screen.width)
                  : const SizedBox.shrink(),
            ),
            BabaBlob(
                size: _activeSize, active: true, pulse: pulse, isDark: isDark),
            _buildLiveControls(state, pulse, isDark),
          ],
        ),
      ),
    );
  ...
  /// The idle blob — draggable ANYWHERE. Rendered as an [AnimatedPositioned] so
  /// it eases when the default anchor shifts (e.g. the tab bar hides) but tracks
  /// the finger 1:1 while dragging. tap = voice, long-press = typing.
  ...
  Widget _buildCaption(String reply, bool isDark, double screenWidth) {
    return Padding(
  ...
  Widget _buildLiveControls(VoiceCallState state, double pulse, bool isDark) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        const SizedBox(height: 14),
        BabaStatusPill(state: state, pulse: pulse, isDark: isDark),
        BabaVoiceControls(
          state: state,
          onEndCall: _voice.hangUp,
          onSwitchToText: _openChat,
