...
/// Thin client for Aurobhatt's durable-memory controls.
///
...
class AurobhattMemoryService {
  AurobhattMemoryService({FirebaseFunctions? functions})
      : _functions =
            functions ?? FirebaseFunctions.instanceFor(region: 'asia-southeast2');

  final FirebaseFunctions _functions;

  /// Wipe the signed-in user's durable Aurobhatt memory. The next conversation
  ...
  Future<void> clearMemory() async {
    try {
      ...
      AppLogger.i('Aurobhatt memory cleared', category: LogCategory.general);
