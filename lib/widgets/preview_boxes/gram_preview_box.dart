...
class _GramPreviewBoxState extends State<GramPreviewBox>
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          // Always show call button (group calls work on all platforms)
                          const SizedBox(width: 8),
                          _buildCallButton(
                            spaceData['id'] as String? ?? '',
                            spaceData['name'] as String? ?? 'Gram',
                          ),
                          // Show chat button and lock icon only when showChatButton is true
                          if (widget.showChatButton) ...[
                            _buildChatAndLockIcons(spaceData, type),
                          ],
                        ],
                      ),
  ...
  Widget _buildChatAndLockIcons(Map<String, dynamic> spaceData, SpaceType? type) {
  ...
  Widget _buildCallButton(String spaceId, String spaceName) {
    if (spaceId.isEmpty) return const SizedBox.shrink();
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final callButton = GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTap: () => _openCall(spaceId, spaceName),
          child: Container(
  ...
  void _openCall(String spaceId, String spaceName) {
