...
class _CrewPreviewState extends State<CrewPreview> {
  ...
  initializePreview() async {
    DocumentSnapshot documents = await usersCollection.doc(widget.user).get();
    name = documents['name'];
    if (this.mounted) {
      setState(() {});
    }
    if (documents['displayPicture'] != '') {
      picture = await DefaultCacheManager()
          .getSingleFile(documents['displayPicture']);
