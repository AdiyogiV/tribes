...
class UserPreview extends StatefulWidget {
  ...
  const UserPreview({this.uid, this.showName, Key? key}) : super(key: key);
  @override
...
class _UserPreviewState extends State<UserPreview> {
  ...
  Future<void> getData() async {
    try {
      // Try to fetch directly from Firestore since we can't access the private method
      ...
      if (authorDocuments.exists) {
        ...
        if (data != null && mounted) {
          ...
          final newUsername = data['nickname'] as String? ?? 'user';
          final newImageUrl = data['displayPicture'] as String? ?? '';
