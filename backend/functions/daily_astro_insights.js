...
// Gemini configuration (replaces OpenRouter for cost savings)
 ...
 */
function extractAscendantDegree(astroData) {
...
async function generateInsightWithAI(userAstroData, todayAstroData, searchContext) {
    ...
    const prompt = `You're a bold astrologer who creates SHAREABLE insights that make users say "THIS is so me!" and want to share with friends.
...
OUTPUT (VALID JSON ONLY):
{
  ...
  "sections": [
    ...
    try {
        ...
        const systemPrompt = `You create SHAREABLE astrology insights that users screenshot and send to friends.
...
async function getTodayAstroData(userAstroData) {
    ...
    try {
        ...
        return { panchang, transits, shadBala, muhurat };
    } catch (error) {
...
// ═══════════════════════════════════════════════════════════════
// OLD DISPATCH INDEX CODE (DEPRECATED - kept for reference)
// ═══════════════════════════════════════════════════════════════

/**
 * Write dispatch index entries for a user's insight cards
 * DEPRECATED: Replaced by Cloud Tasks scheduling
 * @deprecated Use enqueueDispatchTasks instead
 */
async function writeDispatchIndex(userId, date, sections) {
    if (!sections || sections.length === 0) {
        return;
    }
    ...
    for (let i = 0; i < sections.length; i++) {
        const section = sections[i];
        const targetTime = section.scheduledFor || "06:00";

        // Create dispatch entry: insightDispatch/{date}/{targetTime}/{userId_cardIndex}
        const dispatchRef = db
            .collection("insightDispatch")
            .doc(date)
            .collection(targetTime.replace(":", "")) // "06:00" -> "0600"
            .doc(`${userId}_${i}`);
        ...
        batch.set(dispatchRef, {
            ...
            cardType: "insight", // Unified type
            title: section.title || "",
            content: (section.content || "").substring(0, 150), // Preview only
    ...
    logger.info("[DISPATCH-INDEX] Wrote dispatch entries", {
        structuredData: true,
        userId,
        date,
        entriesWritten,
        times: [...new Set(sections.map((s) => s.scheduledFor))],
    });
}

/**
 * Delete dispatch index entries for a user after notifications are sent
 * @param {string} date - Date string (yyyy-MM-dd)
 * @param {string} targetTime - Time slot (e.g., "06:00")
 * @param {string} userId - User ID
 * @param {number} cardIndex - Card index
 */
async function deleteDispatchEntry(date, targetTime, userId, cardIndex) {
    try {
        const dispatchRef = db
            .collection("insightDispatch")
            .doc(date)
            .collection(targetTime.replace(":", ""))
            .doc(`${userId}_${cardIndex}`);

        await dispatchRef.delete();
    } catch (e) {
        // Don't fail if delete fails - entry will be stale but harmless
        logger.warn("[DISPATCH-INDEX] Failed to delete entry", {
            structuredData: true,
            userId,
            cardIndex,
            error: String(e),
...
export const getAstroInsightSystemHealth = onCall({
    ...
    try {
        ...
        } else if (!searchKey || !searchCx) {
            health.status = "limited"; // Works but no search
        }
